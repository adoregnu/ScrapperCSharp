namespace Scrapper.Utils
{
    ...
    internal static class ThumbnailGenerator
    {
        ...
        public static string SnapThumbnail(Image sourceImage, string targetPath)
        {
            ...
            using (var thumb = CreateThumbnail(sourceImage, Color.Black,
        ...
        public static string SaveThumbnail(Image thumbnail, string baseDirectory)
        {
            ...
            var name = $"{Path.GetFileName(baseDirectory)}_thumbnail.png";
            var targetFilename = Path.Combine(Path.GetFullPath(baseDirectory), name);
